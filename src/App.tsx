...
//   // 'NOT_VERIFIED' (휴대폰 미인증) 상태일 경우, phone verification 페이지만 허용합니다.
//   if (role === 'NOT_VERIFIED' && currentPath !== '/verify-phone') {
...
/**
 * [MODIFIED] 로그인한 사용자만 접근할 수 있는 경로를 처리하며,
 * 휴대폰 미인증 사용자도 홈에 접근할 수 있도록 로직을 수정합니다.
 */
function ProtectedRoute({ children }: { children: React.ReactNode }) {
  const location = useLocation();
  const userInfo = getUserInfo();
