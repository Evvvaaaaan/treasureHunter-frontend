  ...
  const loadChatRooms = async () => {
    ...
    try {
      ...
      const uiRoomsPromises = apiRooms.map(async (room: ApiChatRoom) => {
        ...
        try {
          // [수정] 중복 호출 제거 및 로직 단순화
          // 목록용 미리보기를 위해 50개만 가져옵니다. 
          const syncData = await fetchChatMessages(room.roomId, 0, 300);
          const messages = syncData.chats || [];

          if (messages.length > 0) {
            const lastMsg = messages[messages.length - 1]; // 최신 메시지 (순서가 created ASC 가정)
            ...
            if (lastMsg.type === 'IMAGE') {
              ...
              lastMessageText = lastMsg.message || '메시지 없음';
              lastMessageType = 'text';
            }
            timestamp = lastMsg.serverAt || lastMsg.sentAt || timestamp;
            ...
            console.log(`[ChatList] Room ${room.roomId} LastMsg: ${lastMessageText}`);
          } else if (room.lastMessage) {
            // 메시지가 없으면 room 정보에 있는 lastMessage 사용 (fallback)
            lastMessageText = room.lastMessage;
          }
        } catch (e) {
          console.error(`Error loading room ${room.roomId}`, e);
          // 에러 발생 시 room 기본 정보라도 보여줌
          if (room.lastMessage) lastMessageText = room.lastMessage;
        }
        ...
        return {
          ...
          itemImage: room.post?.image, // images 배열 첫번째 처리 필요할 수 있음
